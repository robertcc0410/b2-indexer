...
func TestTransferConfig(t *testing.T) {
	...
	os.Unsetenv("TRANSFER_FAKE_PRIVATE_KEY")
	...
	os.Unsetenv("TRANSFER_FEE_RATE")
	os.Unsetenv("TRANSFER_NOTE")

	config, err := config.LoadTransferConfig("./testdata")
	require.NoError(t, err)
	require.Equal(t, "https://api.sinohope.com", config.BaseUrl)
	require.Equal(t, "", config.FakePrivateKey)
	require.Equal(t, "536375192315653", config.VaultId)
	require.Equal(t, "536377257441029", config.WalletId)
	require.Equal(t, "131fpYjELat58RVzPp2A9Bo8oNuKiP4vxg", config.From)
	...
	require.Equal(t, "2", config.FeeRate)
	require.Equal(t, "", config.Note)
}

func TestTransferConfigEnv(t *testing.T) {
	os.Setenv("TRANSFER_BASE_URL", "https://api.sinohope.com")
	os.Setenv("TRANSFER_FAKE_PRIVATE_KEY", "")
	os.Setenv("TRANSFER_VAULT_ID", "536375192315653")
	os.Setenv("TRANSFER_WALLET_ID", "536377257441029")
	os.Setenv("TRANSFER_FROM", "131fpYjELat58RVzPp2A9Bo8oNuKiP4vxg")
	os.Setenv("TRANSFER_CHAIN_SYMBOL", "BTC_BTC")
	os.Setenv("TRANSFER_ASSET_ID", "BTC")
	os.Setenv("TRANSFER_OPERATION_TYPE", "TRANSFER")
	os.Setenv("TRANSFER_FEE_RATE", "2")
	os.Setenv("TRANSFER_NOTE", "")
